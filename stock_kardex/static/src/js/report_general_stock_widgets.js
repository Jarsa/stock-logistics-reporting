odoo.define('stock_kardex.ReportWidget', function (require) {
'use strict';
...
var ReportWidget = Widget.extend({
    ...
    start: function() {
        QWeb.add_template("/stock_kardex/static/src/xml/kardex_report_stock_line.xml");
    ...
    unfold: function(e) {
        ...
        return this.context_model.call('add_line', [[parseInt(context_id, 10)], parseInt(active_id, 10)]).then(function () { // First add the line to the model
            ...
            if (!isLoaded) { // If the lines have not yet been loaded
                ...
                new Model('kardex.report.context.common').call('get_full_report_name_by_report_name', [report_name]).then(function (result) {
                    ...
                    var f = function (lines) {// After loading the line
                        self.context_model.call('get_columns_types', [[parseInt(context_id, 10)]]).then(function (types) {
                            ...
                            for (line in lines) { // Render each line
                                $cursor.after(QWeb.render("report_kardex_stock_line", {l: lines[line], types: types}));
                                $cursor = $cursor.next();
                            }
                        });
                    };
                    if (report_name === 'manager_ledger') { // Fetch the report_id first if needed
                        self.context_model.query(['report_id'])
                        .filter([['id', '=', context_id]]).first().then(function (context) {
                            reportObj.call('get_lines', [[parseInt(context_id, 10)], parseInt(context_id, 10), parseInt(active_id, 10)], {context : self.odoo_context}).then(f);
                        });
                    }
                    else {
