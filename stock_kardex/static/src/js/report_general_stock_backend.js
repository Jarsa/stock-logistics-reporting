odoo.define('stock_kardex.general_report_generic', function (require) {
'use strict';
...
var ReportWidget = require('stock_kardex.ReportWidget');
...
var general_report_generic = Widget.extend(ControlPanelMixin, {
    ...
    get_html: function() {
        var self = this;
        var defs = [];
        return new Model('kardex.report.context.common').call('return_context', [self.report_model, self.given_context, self.report_id]).then(function (result) {
    ...
    render_buttons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("kardexReports.buttons", {xml_export: this.xml_export}));
    ...
    render_searchview_buttons: function() {
        ...
        this.$searchview_buttons = $(QWeb.render("kardexReports.searchView", {report_type: this.report_type, context: this.report_context}));
        ...
        if (this.report_model == 'kardex.manager.ledger') { // Same for the tags filter
            this.$searchview_buttons.find(".o_product_reports_filter_product_auto_complete").select2();
            var selection = [];
            for (var i = 0; i < this.report_context.filter_product_ids.length; i++) { 
                var filter_product = this.report_context.filter_product_ids[i];
                selection.push({id:filter_product[0], text:filter_product[1]});
            }
            this.$searchview_buttons.find('.o_product_reports_filter_product_auto_complete').data().select2.updateSelection(selection);
            this.$searchview_buttons.find('.o_account_reports_add_filter_product_tag').bind('click', function (event) {
                var report_context = {};
                var value = self.$searchview_buttons.find(".o_product_reports_filter_product_auto_complete").select2("val");
                report_context.filter_product_ids = value;
                self.restart(report_context);
            });
        }
